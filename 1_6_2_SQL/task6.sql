...
-- Мои решения от эталонных отличаются тем, что на выходе возвращают json-массив, который можно как есть вернуть http клиенту.
-- Эталонные решения можно на стороне сервиса намаппить на ResultSet, а затем и на ORM-представление с полем типа JsonNode для 'related_entities'
-- и с возможностью сбора статистики по метрикам. К эталонным решениям также можно частично применить строгую типизацию с валидацией схемы,
-- так как только поле 'related_entities' создается динамически.
-- Таким оборазом, мое решение подойдет для сценариев, когда база данных также является и сервером, куда приходят сетевые вызовы,
-- а эталонное решение подойдет для классической 3-tier архитектуры с бОльшим разделением на независимые функциональные блоки