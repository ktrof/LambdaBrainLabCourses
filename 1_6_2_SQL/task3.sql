...
-- 3. SELECT DISTINCT + JOIN и EXISTS() эквивалентны в случае проверки существования в связанной таблице https://de.ifmo.ru/--books/sql/3-3-6.html
-- 4. Эталонный запрос возвращается сгруппированное по статусам количество задач, которые есть у гномов. Если потребуется выбрать даже задачи в нулевом количестве по всем гномам,
-- то надо будет делать декартово произведение дномов на статусы и применять к нему левый джоин по id гнома и статусу
